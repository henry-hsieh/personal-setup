...
release: _generate_exclude
    #!/usr/bin/env bash
    if [[ "$(git rev-parse --is-inside-work-tree >/dev/null 2>&1 && echo true)" == "true" ]]; then
        VERSION=$(git describe --tag --exact --exclude=\"nightly*\" 2> /dev/null || git rev-parse --short HEAD)
    else
        VERSION="unknown"
    fi
...
test: _test_setup && _test_command
    chmod +x {{build_dir}}/personal-setup.xz.sh
